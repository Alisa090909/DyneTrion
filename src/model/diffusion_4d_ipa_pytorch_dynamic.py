...
def _run_ipa_block(module, all_node, all_edge_embed, all_curr_rigids, all_node_mask):
    return module(all_node, all_edge_embed, all_curr_rigids, all_node_mask)
...
class DFOLDIpaScore(nn.Module):
    ...
    def forward(self, node_embed, edge_embed, input_feats,drop_ref=False,is_training=True):
        ...
        for b in range(self._ipa_conf.num_blocks):
            ...
            # if drop_ref:
            #     spatial_node_with_ref = torch.cat([torch.zeros_like(ref_node), frame_node],dim=0) # 33, 155, 256
            #     spatial_node_init_with_ref = torch.cat([torch.zeros_like(ref_node_init), frame_node_init],dim=0) # 33, 155, 256
            # else:
            # here compute ref and frame node 
            # through the reference Net
            if self._ipa_conf.spatial:
                spatial_node_with_ref = torch.cat([ref_node,frame_node],dim=0) # 33, 155, 256
                spatial_node_init_with_ref = torch.cat([ref_node_init,frame_node_init],dim=0) # 33, 155, 256
            ...
            if self._ipa_conf.temporal:
                ...
                # ref node should removed here
                temporal_node = torch.concat([motion_node, frame_node])#35, 155, 256
                temporal_node_init_with_motion = torch.cat([motion_node_init,frame_node_init],dim=0)#35, 155, 256
                # TODO
                # here to add the sin-cos embeddings for frame
                seq_tfmr_in = torch.cat([temporal_node, self.trunk[f'skip_temporal_embed_{b}'](temporal_node_init_with_motion)], dim=-1)#35,155, 320
                seq_tfmr_in = self.trunk[f'seq_tfmr_temporal_position_encoding_{b}'](seq_tfmr_in.permute([1,0,2]))
                # run temporal alignment
                seq_tfmr_out = self.trunk[f'seq_tfmr_temporal_{b}']( seq_tfmr_in, src_key_padding_mask=1 - node_mask.repeat(self.motion_number+self.frame_time,1).permute([1,0])) #torch.Size([58,4,  640])
