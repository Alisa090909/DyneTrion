...
class SE3Diffuser:
    ...
    def reverse(self, rigid_t, rot_score, trans_score, t, dt, diffuse_mask=None, center=True, noise_scale=1.0, device=torch.device('cpu')):
        # 提取：全是 Tensor 且在 GPU 上
        trans_t, rot_t = _extract_trans_rots(rigid_t)
        
        # 旋转更新：调用 so3 扩散器 (去掉.numpy() )
        rot_t_1 = self._so3_diffuser.reverse(
            rot_t=rot_t,
            score_t=rot_score,
            t=t,
        ...
        if diffuse_mask is not None:
            trans_t_1 = diffuse_mask[..., None] * trans_t_1 + (1 - diffuse_mask[..., None]) * trans_t
            rot_t_1 = diffuse_mask[..., None] * rot_t_1 + (1 - diffuse_mask[..., None]) * rot_t
