...
from openfold.utils import rigid_utils0209 as ru
...
class Experiment:
    ...
    def __init__(
        ...
        with torch.no_grad():
            ...
            # Self-conditioning 的初始 Scaling (reverse_steps[0] )
            self.sc_rot_scale = self.all_rot_scales[0]
            self.sc_trans_scale = self.all_trans_scales[0]
            self.sc_t = self.reverse_steps[0]
            
            # self.all_t_tensors = self.reverse_steps.unsqueeze(-1) * self.t_placeholder
            # self.all_rot_scale_tensors = all_rot_s.unsqueeze(-1) * self.t_placeholder
            # self.all_trans_scale_tensors = all_trans_s.unsqueeze(-1) * self.t_placeholder

            # # 预存 Self-conditioning 专用（即第 0 步）
            # self.sc_t_tensor = self.all_t_tensors[0]
            # self.sc_rot_scale_tensor = self.all_rot_scale_tensors[0]
            # self.sc_trans_scale_tensor = self.all_trans_scale_tensors[0]
    ...
    def _process_one_protein_extrapolation(
        ...
        # print(f'frame_time, L:=={frame_time}=={L}')

        # 预计算 16 帧外推“初始态”(t=1.0)
